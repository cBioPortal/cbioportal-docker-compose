...
def _validate_release_target(args: argparse.Namespace, prefixes: tuple[str, ...]) -> None:
    if not args.release_mode:
        return
    if not args.active_database:
        raise RuntimeError("--release-mode requires --active-database")
    if args.database == args.active_database:
        raise RuntimeError("release hydration target must not be the active database")
    if set(prefixes) != set(exporter._DEFAULT_SOURCE_PREFIXES):
        raise RuntimeError("release hydration prefixes differ from the approved serving policy")
    if not args.populate_derived_tables_sql:
        raise RuntimeError("--release-mode requires --populate-derived-tables-sql")
    if not args.study_ids:
        raise RuntimeError("--release-mode requires at least one --study-id")
...
def _args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        dest="study_ids",
        action="append",
        help="study to hydrate completely; repeat for each release study",
    )
    p.add_argument(
        "--active-database",
        default=os.environ.get("ACTIVE_CLICKHOUSE_DB", ""),
        help="currently served database; required with --release-mode",
    )
    p.add_argument(
        "--release-mode",
        action="store_true",
        default=os.environ.get("WSI_RELEASE_MODE", "0") == "1",
        help="enforce inactive-database and complete-publication safety gates",
    )
    p.add_argument(
        "--populate-derived-tables-sql",
        type=Path,
        default=os.environ.get("POPULATE_DERIVED_TABLES_SQL", ""),
        help="candidate backend populate_derived_tables.sql",
    )
...
def _load_target_maps(args: argparse.Namespace, selected_studies: set[int] | None = None) -> tuple[dict[str, list[tuple[int, int]]],
    ...
    for stable, internal, study in patient_rows:
        ...
        if selected_studies is not None and study_id not in selected_studies:
            continue
...
def _insert_timeline(
    ...
    return len(events), {event[0] for event in events}
...
def main() -> int:
    ...
    _validate_release_target(args, prefixes)
    ...
    study_id_by_stable = {stable: internal for internal, stable in study_stable_by_id.items()}
    requested_studies = set(args.study_ids or [])
    unknown_studies = sorted(requested_studies - set(study_id_by_stable))
    if unknown_studies:
        raise RuntimeError(
            "requested studies are absent from the target database: "
            + ", ".join(unknown_studies)
        )
    requested_internal_ids = (
        {study_id_by_stable[stable] for stable in requested_studies}
        if requested_studies
        else None
    )
    patient_targets, sample_targets, patient_stable_by_internal = _load_target_maps(
        args, requested_internal_ids
    )
    ...
    try:
        ...
        if requested_internal_ids is not None and set(study_ids) != requested_internal_ids:
            missing = sorted(
                study_stable_by_id[study]
                for study in requested_internal_ids - set(study_ids)
            )
            raise RuntimeError(
                "Databricks WSI data is missing requested studies: " + ", ".join(missing)
            )
        servable_studies: set[int] = set()
        for study_id, values_json in db.execute("SELECT study_id, values_json FROM slides"):
            values = json.loads(values_json)
            if values[DATA_COLUMNS.index("CAN_SERVE_TILES")] == "TRUE":
                servable_studies.add(int(study_id))
        missing_servable = sorted(
            study_stable_by_id[study] for study in set(study_ids) - servable_studies
        )
        if missing_servable:
            raise RuntimeError(
                "requested studies have no servable WSI assets: "
                + ", ".join(missing_servable)
            )
        ...
        timeline_count, timeline_studies = _insert_timeline(args, db, study_stable_by_id)
        if timeline_count <= 0:
            raise RuntimeError("hydration produced no pathology timeline events")
        missing_timeline = sorted(
            study_stable_by_id[study] for study in set(study_ids) - timeline_studies
        )
        if missing_timeline:
            raise RuntimeError(
                "requested studies have no pathology timeline events: "
        ...
        populate = args.populate_derived_tables_sql
        if not populate:
            candidates = (
                Path(__file__).resolve().parents[2]
                / "cbioportal"
                / "src/main/resources/db-scripts/clickhouse/populate_derived_tables.sql",
                Path(__file__).resolve().parents[2]
                / "cbioportal-prod-migration-rehearsal"
                / "src/main/resources/db-scripts/clickhouse/populate_derived_tables.sql",
            )
            populate = next((path for path in candidates if path.is_file()), None)
        if populate is None or not Path(populate).is_file():
            raise RuntimeError(
                "candidate populate_derived_tables.sql was not found; pass "
                "--populate-derived-tables-sql"
            )
        command = [args.clickhouse_bin, "client", "--config-file", str(args.clickhouse_config),
        ...
        audit = _query_rows(
            ...
            "countIf(isNull(slide_type) OR slide_type NOT IN ('H&E','IHC','Other')), "
            "countIf((is_hne AND is_ihc) OR (slide_type='H&E' AND NOT is_hne) "
            "OR (slide_type='IHC' AND NOT is_ihc) OR "
            "(slide_type='Other' AND (is_hne OR is_ihc))) "
            f"FROM wsi_slide WHERE cancer_study_id IN ({_sql_ids(study_ids)}) FORMAT TSV",
        )
        total, servable, invalid_type, invalid_flags = map(int, audit[0])
        if total != int(stats["selected_rows"]):
            raise RuntimeError(
                f"hydrated WSI row count differs from staging: staged={stats['selected_rows']} loaded={total}"
            )
        if servable <= 0 or invalid_type or invalid_flags:
            raise RuntimeError(
                "hydrated WSI audit failed: "
                f"servable={servable} invalid_type={invalid_type} invalid_flags={invalid_flags}"
            )
