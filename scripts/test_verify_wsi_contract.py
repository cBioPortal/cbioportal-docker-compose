...
class PortalTileContractTests(unittest.TestCase):
    ...
    def test_release_verifiers_use_prod_tables_and_complete_slide_checks(self):
        self.assertIn('databricks_target="${DATABRICKS_TARGET:-prod}"', E2E_SCRIPT)
        self.assertIn(
        ...
        self.assertIn('${VERIFY_ALL_TILES:-1}', E2E_SCRIPT)
        self.assertIn('${VERIFY_ALL_TILES:-1}', STACK_SCRIPT)

    def test_immutable_release_rejects_mutable_frontend_aliases(self):
        manifest = {
            "version": 2,
            ...
            "components": {
                "frontend": {
                    "git_sha": "0" * 40,
                    "artifact_url": "https://6aa8bf256dfcb5000872a8e9--cbioportalfrontend.netlify.app/",
                },
                "backend": {"git_sha": "1" * 40, "image_digest": "portal@sha256:" + "2" * 64},
                "tile_server": {"git_sha": "3" * 40, "image_digest": "tile@sha256:" + "4" * 64},
                "pdm": {
                    "git_sha": "5" * 40,
                    "contract_version": "wsi-serving-v2",
                    "databricks_job_run_id": "123",
                },
                "clickhouse": {"schema_version": "3.2.0", "source_database": "prod_clone"},
            },
            "allowed_source_prefixes": sorted(STACK.REQUIRED_SOURCE_PREFIXES),
            "allowed_thumbnail_prefixes": sorted(STACK.REQUIRED_THUMBNAIL_PREFIXES),
            "studies": [],
        }
        self.assertEqual(STACK._validate_release_identity(manifest)["release_id"], "candidate-1")
        manifest["components"]["frontend"]["artifact_url"] = (
            "https://deploy-preview-5608--cbioportalfrontend.netlify.app/"
        )
        with self.assertRaisesRegex(STACK.VerificationError, "mutable PR preview"):
            STACK._validate_release_identity(manifest)

    def test_live_release_identity_checks_portal_and_tile_runtime(self):
        manifest = {
            "release_id": "candidate-1",
            "components": {
                "frontend": {
                    "artifact_url": "https://immutable--frontend.example.test/",
                },
                "backend": {"git_sha": "1" * 40},
                "tile_server": {"git_sha": "2" * 40},
            },
        }
        responses = [
            {
                "frontendUrl": "https://immutable--frontend.example.test",
                "wsi_release_id": "candidate-1",
                "wsi_backend_git_sha": "1" * 40,
                "wsi_serving_contract_version": "wsi-serving-v2",
                "msk_wsi_tile_server_url": "https://tiles.example.test/api/v1",
            },
            {
                "release_id": "candidate-1",
                "image_git_sha": "2" * 40,
                "serving_contract_version": "wsi-serving-v2",
            },
        ]
        with mock.patch.object(STACK, "_request_json", side_effect=responses) as request:
            STACK._validate_live_release_identity(
                "https://portal.example.test", "session=cookie", manifest
            )
        self.assertEqual(
            request.call_args_list[1].args,
            ("https://tiles.example.test/ready", "session=cookie"),
        )

    def test_live_release_identity_rejects_stale_backend(self):
        manifest = {
            "release_id": "candidate-1",
            "components": {
                "frontend": {"artifact_url": "https://frontend.example.test/"},
                "backend": {"git_sha": "1" * 40},
                "tile_server": {"git_sha": "2" * 40},
            },
        }
        with mock.patch.object(
            ...
            return_value={
                ...
                "wsi_release_id": "candidate-1",
                "wsi_backend_git_sha": "stale",
                "wsi_serving_contract_version": "wsi-serving-v2",
            },
        ):
            with self.assertRaisesRegex(STACK.VerificationError, "wsi_backend_git_sha"):
                STACK._validate_live_release_identity(
                    "https://portal.example.test", "", manifest
                )
...
class ReleaseManifestTests(unittest.TestCase):
    def test_release_hydration_refuses_the_active_database(self):
        args = Namespace(
            release_mode=True,
            active_database="cbioportal_blue",
            database="cbioportal_blue",
            populate_derived_tables_sql=Path("populate.sql"),
        )
        with self.assertRaisesRegex(RuntimeError, "must not be the active database"):
            EXPORTER_HYDRATE._validate_release_target(
                args, EXPORT._DEFAULT_SOURCE_PREFIXES
            )

    def test_release_hydration_requires_explicit_studies(self):
        args = Namespace(
            release_mode=True,
            active_database="cbioportal_blue",
            database="cbioportal_green",
            populate_derived_tables_sql=Path("populate.sql"),
            study_ids=[],
        )
        with self.assertRaisesRegex(RuntimeError, "at least one --study-id"):
            EXPORTER_HYDRATE._validate_release_target(
                args, EXPORT._DEFAULT_SOURCE_PREFIXES
            )
