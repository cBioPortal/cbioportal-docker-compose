...
Optional environment:
  ...
  VERIFY_ALL_TILES           set to 0 to skip every slide's pixel path (default: 1)
...
The manifest must use version 2 and pin immutable frontend, backend,
tile-server, PDM, Databricks, ClickHouse, and per-study snapshot identities.
Every study must contain a checksummed WSI snapshot with non-zero association,
servable, patient, and timeline counts. Incomplete releases are rejected.
USAGE
  exit 0
fi
