...
The gate requires a version 2 immutable release manifest, an exact
catalog/manifest match, and matching frontend, backend, tile-server, PDM, and
ClickHouse release identities. It checks every patient hierarchy and every
slide association, compares WSI clinical counts with ClickHouse, and requests
the access bundle, thumbnail, and one real pixel tile for every servable
slide. Set `VERIFY_ALL_TILES=0` only for local diagnosis; that reduced run is
not an accepted release verification. The portal's live `config_service` tile
URL is authoritative; the
...
default release run uses the production Databricks tables and checks every
patient hierarchy, every servable access bundle and thumbnail, and one pixel
tile per servable slide. It requires the full PDM git SHA, Databricks job run
ID, and both Delta table versions so the generated snapshot can be traced to
the exact producer and inputs. Set `VERIFY_ALL_ACCESS=0` or
`VERIFY_ALL_TILES=0` only for local diagnosis; such a reduced run is not an
accepted release verification. Set `KEEP_E2E_STAGE=1` to retain the generated
