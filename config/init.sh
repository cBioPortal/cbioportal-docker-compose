...
docker run --rm -i --env-file ../.env.temp $VERSION bin/sh -c 'cat /cbioportal-webapp/application.properties |
    sed "s|spring.datasource.password=.*|spring.datasource.password=${DB_CLICKHOUSE_PASSWORD}|" | \
    sed "s|spring.datasource.username=.*|spring.datasource.username=${DB_CLICKHOUSE_USERNAME}|" | \
