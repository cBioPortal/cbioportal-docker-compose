...
if version_is_older_or_equal "$detected_clickhouse_version" "25.6.x" ; then
    ...
    if version_is_older_or_equal "$detected_clickhouse_version" "26.2.x" ; then
        execute_clickhouse_statement "GRANT READ ON REMOTE TO ${CP_CLICKHOUSE_USER_FOR_CBIOPORTAL_DB_UPDATE}"
    else
        echo "Warning : initialization of clickhouse versions after 26.2 has not yet been evaluated. Problems are not expected, but be advised that version '$ver' has not yet been tested." >&2
        execute_clickhouse_statement "GRANT READ ON REMOTE TO ${CP_CLICKHOUSE_USER_FOR_CBIOPORTAL_DB_UPDATE}"
    fi
fi
